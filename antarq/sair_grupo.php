...
if(isset($_POST["cadastrar"])) { // Verifico se o botão cadastrar foi acionado
	...
	if(!empty($_POST["us1"]) && !empty($_POST["us1name"])) { // Verifico se os campos foram preenchidos
		...
		$repeat_user = mysql_query("SELECT * FROM us_grupo WHERE us='$id_gen';") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysql_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
		$repeat_user1 = mysql_query("SELECT * FROM users1 WHERE id='$id_gen' AND senha='$senha';") or die($mensagem_erro = "<b>Houve um erro:</b><br />".mysql_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
